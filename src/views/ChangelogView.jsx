...
import { APPS, STATUS_COLORS } from '../constants';
import TypeBadge from '../components/TypeBadge';

function ChangelogView({ posts }) {
  const { t } = useTheme();
  const completed = posts.filter((p) => p.status === "done" && p.completed_at).sort((a, b) => new Date(b.completed_at) - new Date(a.completed_at));
  return (
